...
import type { Widget, WidgetPosition, Dashboard, WidgetConfig } from '../types/index.js';
...
interface DashboardWidgetsResponse {
    ...
    is_personal?: boolean;
    is_global?: boolean;
    can_view_widgets?: boolean;
    can_update_widgets?: boolean;
...
export const DashboardProvider = ({ children }: { children: preact.ComponentChildren }) => {
    ...
    const [sharedDashboardMode, setSharedDashboardMode] = useState<string>('load');
    const [editingWidget, setEditingWidget] = useState<Widget | undefined>(undefined);
    const [authzError, setAuthzError] = useState<string | undefined>(undefined);
    const [unsavedChanges, setUnsavedChanges] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [permissions, setPermissions] = useState<DashboardPermissionsState>({
        canEdit: false,
        canEditGlobal: false,
        isPersonal: false,
        isGlobal: false,
    ...
    const loadDashboard = useCallback(async () => {
        setIsLoading(true);
        try {
            const result = await api.fetch('/dashboards/widgets') as ApiResult<DashboardWidgetsResponse>;
            if (result.success) {
    ...
    const loadDashboardById = useCallback(async (dashboardId: string | number) => {
        setIsLoading(true);
        try {
            const result = await api.fetch(`/dashboards/${dashboardId}/widgets`) as ApiResult<DashboardWidgetsResponse>;
            if (result.success) {
                const nextDashboard = result.data?.dashboard;
                const nextWidgets = result.data?.widgets || [];
    ...
    const loadGlobalDashboard = useCallback(async () => {
        setIsLoading(true);
        try {
            const result = await api.fetch('/dashboards/global/widgets') as ApiResult<DashboardWidgetsResponse>;
            if (result.success) {
                const nextDashboard = result.data?.dashboard;
                const nextWidgets = result.data?.widgets || [];
    ...
    const loadDashboards = useCallback(async () => {
        try {
            const result = await api.fetch('/dashboards') as ApiResult<{ dashboards?: Dashboard[] } | Dashboard[]>;
            if (result.success) {
    ...
    const createPersonalDashboard = useCallback(async (name: string, sourceDashboardId?: string | number) => {
        try {
            const result = await api.post('/dashboards/personal', {
                name: name || 'My Dashboard',
                source_dashboard_id: sourceDashboardId
            }) as ApiResult<{ dashboard?: Dashboard }>;
            if (result.success) {
    ...
    const createSharedDashboard = useCallback(async (name: string, sourceDashboardId?: string | number) => {
        try {
            ...
            if (result.success) {
                return (result as { dashboard?: Dashboard }).dashboard || result.data?.dashboard || result.data;
            }
            return ;
        } catch (error) {
    ...
    const saveDashboard = useCallback(async () => {
        ...
        try {
            ...
            }) as ApiResult<DashboardWidgetsResponse>;
    ...
    const saveWidgetPositions = useCallback(async (positions: WidgetPosition[]) => {
    ...
    const addWidget = useCallback(async (widgetData: WidgetDataInput) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                let updatedWidgets: Widget[] = result.data?.widgets || [];
                ...
                if (result.data?.dashboard_id && result.data?.dashboard_id !== dashboard?.id) {
                    const dashResult = await api.fetch('/dashboards/widgets') as ApiResult<DashboardWidgetsResponse>;
    ...
    const createCustomWidget = useCallback(async (widgetData: WidgetDataInput | WidgetConfig) => {
        const config = (widgetData as WidgetDataInput)?.config ?? widgetData as WidgetConfig;
        if (!config) {
            return false;
        }

        const defaultWidth = (widgetData as WidgetDataInput)?.default_width ?? (widgetData as WidgetDataInput)?.width ?? 4;
        const defaultHeight = (widgetData as WidgetDataInput)?.default_height ?? (widgetData as WidgetDataInput)?.height ?? 4;

        try {
            const result = await api.post('/widgets/create', {
                config,
                widget_type: (widgetData as WidgetDataInput)?.widget_type ?? 'custom',
                add_to_dashboard: false,
                default_width: defaultWidth,
                default_height: defaultHeight
            }) as ApiResult<{ widget_id?: string | number }>;
    ...
    const updateWidget = useCallback(async (widgetId: string | number, updates: { config?: WidgetConfig } | WidgetConfig) => {
        const configUpdates = (updates as { config?: WidgetConfig })?.config ?? updates as WidgetConfig;
    ...
    const openWidgetConfig = useCallback((widget: Widget | undefined = undefined) => {
    ...
    const openSharedDashboard = useCallback((mode: string = 'load') => {
